...
├── BOUNDS.md                      # canonical agent contract (generated by `bounds agent --sync`)
...
def run_agent(project_root, *, mode: str, only: set[str] | None = None) -> dict
    # mode "sync"   → write BOUNDS.md + per-agent configs (dedicated files overwritten;
    #                 shared files get a marked block; hand-written configs left untouched).
    #                 → {created, updated, skipped_custom, bounds_md}
