...
- **NodeNext/ESM emitted specifiers went unresolved (BOUNDS-027).** TypeScript's `NodeNext` module resolution requires source to import a sibling TS module by its *emitted* specifier — `./pricing.js` on disk is `pricing.ts`. `_candidate_stems` correctly stopped splitting dots (so `auth.service.ts` resolves), but that left the trailing `.js` on the stem, and `known_noext` is keyed extension-less, so the stem matched nothing and the edge was dropped. In a NodeNext codebase this discards nearly every local edge: a real Fastify/TS backend reported **749 of ~1,500 relative imports unresolved with `extraction_failures=0`** — parsed fine, resolved to nothing — leaving cycle detection and blast-radius operating on a half-blind graph and giving users a standing reason to keep `enforce: off`. A de-suffixed candidate (`.js`/`.jsx`/`.mjs`/`.cjs` → stem) is now tried *after* the exact stem, so a genuine on-disk `.js` file is never shadowed. Same repository: **749 → 72 unresolved (−90%)**, cycle results unchanged (still zero), confirming the recovered edges are real rather than noise.
