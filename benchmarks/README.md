...
benchmarks/
  ...
  dogfood.py         # dogfood harness: coverage + token economics on THIS repo (`make benchmark`)
  ...
  results/
    dogfood-claude.md           # living proof: `make benchmark` output on this repo (coverage + tokens)
    agentab-supermemory-sonnet.md # real-agent A/B (supermemory): with vs without Bounds, measured
    ...
    oss-token-economics-archived.md # ARCHIVED — original 2-repo estimate run (superseded; do not cite)
...
- **Dogfood (this repo):** `make benchmark` — prints the mapping-coverage block
  ...
  into a results file. (`make benchmark` == `python benchmarks/dogfood.py`.)
...
`dogfood.py` reports per-command token counts (Bounds output vs source-equivalent)
...
as if it were the per-`describe` number. See `results/dogfood-claude.md` for the
...
spec** — don't read latency as a comparison axis between submissions. `dogfood.py`
...
python benchmarks/agent_ab.py --repo <repo> --render-only results/agentab-supermemory-sonnet.raw.jsonl  # re-render
```

First run (`supermemory`, Sonnet, 6 tasks × 3 reps — see
[`results/agentab-supermemory-sonnet.md`](results/agentab-supermemory-sonnet.md)): **−60% tokens, −71% cost, −81%
...
One flow for everyone, three benchmark **kinds**. Pick the kind, run its one command, copy
[`TEMPLATE.md`](TEMPLATE.md), fill the five standard sections, and open a PR adding **only** your
file. Never edit someone else's result; add your own. **Always state your tokenizer.**

**Which benchmark when:**

| You want to… | Kind | Command | Result file |
|--------------|------|---------|-------------|
| Re-measure **this** repo | `dogfood` | `make benchmark` | `dogfood-<model>.md` |
| Test a **third-party** repo | `oss` | `make oss-bench REPO=<clone> [NAME=… LANG_LABEL=…]` | `oss-<repo-or-topic>.md` |
| Prove **agent behavior** with vs without Bounds | `agentab` | `make agent-ab REPO=<repo> [REPS=3 MODEL=…]` | `agentab-<repo>-<model>.md` |

**Naming (one rule):** `<kind>-<discriminator>.md` — `kind` ∈ {`dogfood`, `oss`, `agentab`,
`submission`}; pair raw data by stem (`*.raw.jsonl`). **Format (one layout):** every result file
uses the five sections in `TEMPLATE.md` — `Environment` (table) · `Headline` · `Results` ·
`Caveats & honesty` · `Reproduce`. The machine harnesses emit a finished block you paste into
`## Results`.

### ⭐ Please contribute OSS results — this is the most valuable kind

The headline numbers only mean something if they hold on code we **didn't** write. An `oss`
result is the single most useful contribution: it's **one command, zero LLM cost, fully
reproducible**, and it proves Bounds works (and fails soft) on real third-party code.

```bash
git clone --depth 1 <repo-url> /tmp/bench-clone     # shallow; it's a throwaway
make oss-bench REPO=/tmp/bench-clone NAME=<name> LANG_LABEL=<lang>
# paste the printed block into results/oss-<name>.md (TEMPLATE.md), open a PR
rm -rf /tmp/bench-clone
```

It bootstraps a fresh `.bounds/` **inside the clone only**, runs the full command surface, and
prints coverage + token economics + command-surface health. Report any leaked traceback as a bug
(the fail-soft contract says there should be none, even on an unsupported language).

**Repos we'd love covered** (anything not already in `results/` — diversity of language, size, and
layout is the point): a large monorepo; a Go / Rust / Java / Ruby / C# project (to exercise
unsupported-language fail-soft); a flat single-package library; a deeply-nested framework. Already
covered: click, flask, requests, rich, httpx (py); zod, chalk (ts); axios, express, commander,
lodash (js); cobra (go), fd (rust) — see the `oss-*` files. Pick something new.

Contributors can also add qualitative `agentab`/`submission` notes (e.g. "`bounds preflight` caught
a boundary break before commit") — be specific about the task and model.
...
| [`oss-token-economics-archived.md`](results/oss-token-economics-archived.md) | (ARCHIVED — superseded) | Claude (Opus 4.8) | char/4 estimate |
