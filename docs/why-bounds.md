...
| Validate every language | **Python, TypeScript/JS, and SQL migrations** are tree-sitter-verified today. Go/Rust/Java are roadmap. Unsupported files fall back to YAML-only metadata, and *only* for files a manifest names directly — auto-discovered files in an unsupported language are skipped, not validated. |
...
| Only Python + TS/JS + SQL are verified | **v0.2**: Go and Rust adapters. **v0.3**: Java. |
