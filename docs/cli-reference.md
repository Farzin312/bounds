...
|---------|-----------------|
| `bounds guide` | Read-only setup checklist: the ordered steps (init → discover → agent --sync → ci) each marked done/to-do from what's already present, the daily commands, and the single `next` action. Safe to run anywhere; never mutates |
...
| `bounds overview` | Project dashboard: `project`, subsystem count, `roles`/`criticality` breakdown, dependency `edges` (from/to/interfaces), `cycles`, `schema_issues`, and a `health` summary — `ok` (true **only** when a live validation pass is clean AND there are no cycles/schema errors, so the dashboard can't read "healthy" while `validate` would block), `schema_errors`, `cycles`, and a `validation` block (`ok`, `errors`, `warnings`, `structural_drift`, `boundary_violations`, `contract_gaps`, `stale_interfaces`, `mapped_pct`) folded from a real `validate` run |
