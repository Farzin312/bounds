...
|---------|-----------------|
| `bounds guide` | Read-only setup checklist: the ordered steps (init → discover → coverage → agent --sync → ci) each marked done/to-do from what's already present, the daily commands, and the single `next` action. The **optional features** section is always visible, surfacing SDD and agent-invocation configuration with their one-command paths. `--sdd` previews the SDD phase track; when root.yaml has `sdd.enabled: true`, the SDD track appears by default. Safe to run anywhere; never mutates |
...
| `bounds edit <name>` | Safely update one existing subsystem without opening raw manifests. Supports `--description`, repeatable `--path`, `--role`, or `--criticality` (one field per invocation) |
