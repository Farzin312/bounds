...
| `bounds sdd` | Read-only SDD command map. Bare/`--status` returns configured phases and commands; `--phase <name>` returns one command and says whether that phase is configured; `--doctor` checks configuration plus full/quick/preflight readiness. Bounds does not infer prose-phase completion |
