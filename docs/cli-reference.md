...
| `bounds discover` | Auto-generate candidate manifests from un-bounded source. On a repo that already has `.bounds/`, discovery targets only currently unmapped source and linked tests, so it adds coverage without creating duplicate alternate subsystems over owned files; use `bounds calibrate` to reconcile drift in existing manifests. `--apply`, `--namespace <ns>`, `--merge-into 'name=p1,p2'` |
