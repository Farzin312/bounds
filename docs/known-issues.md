...
- **Root cause:** the drift check had no per-file generated signal at the point it compared the exported surface against `exposes`.
- **Fix:** `feat/sdd-integration` — cache records now persist a deterministic `generated` flag (STATE_VERSION `4`), the validation engine threads cached/generated files into `CheckContext`, and structural drift skips undeclared exports from generated files. Calibrate and validate now agree without rereading unchanged source on the quick path.
- **Test:** `tests/extract/test_ignore.py` (compiled-language generated headers detected; head-only scan guards false positives); `tests/cache/test_cache_sqlite.py::test_roundtrip_preserves_generated_flag`; `tests/validate/test_checks.py::test_drift_excludes_generated_file_exports`.

### BOUNDS-022 — unsupported-language hand-authored exposes were lost by `calibrate` and flagged drift by `validate`
- **Severity / Status:** high / **Fixed**
- **Found:** 2026-06-02 via review of unsupported/compiled-language handling (empirical: a Go subsystem with hand-authored `exposes: [Charge, Refund]`).
- **Affected:** `calibrate`/`validate`/`preflight` on any subsystem owning an unsupported-language file (Go, Rust, Java, …) with hand-authored `exposes`.
- **Symptom:** (1) **data loss** — `calibrate --apply` proposed `remove_exposes: [Charge, Refund]` because tree-sitter found nothing ("declared but not actual"), silently deleting a hand-authored surface; (2) **inconsistency** — `validate` flagged those same exposes as `E_STRUCTURAL_DRIFT`, so `calibrate` and `validate` *disagreed* about the manifest (a violation of the shared-resolver invariant).
- **Root cause:** for a language with no adapter, Bounds extracts zero symbols, so every declared expose looked "missing." Neither path distinguished "verified gone" from "unverifiable (no adapter)."
- **Fix:** `harden-agent-ux-overlap-benchmarks` — a shared signal `scan.subsystems_with_unsupported_source` (directory/extension walk, no source reads, ignore-aware, test-file-aware) names subsystems owning an unsupported-language source file. `calibrate._calibrate_one` routes those subsystems' declared-but-unfound exposes to `needs_review` (never `remove_exposes`); `validate.check_structural_drift` skips the declared-but-missing branch for them (signal threaded via `CheckContext.unsupported_owners`, computed by the engine from the **same** helper). The two now agree. A pure supported-language subsystem with a genuinely stale expose is unaffected (still removed / still flags). Verified on the Go fixture: calibrate `remove_exposes` for `payments` `[Charge, Refund]`→`[]` (now `needs_review`); validate `payments` `E_STRUCTURAL_DRIFT`→none (only the honest `E_COVERAGE_GAP` remains).
- **Test:** `tests/discover/test_calibrate.py::test_calibrate_never_removes_unsupported_language_exposes`, `::test_calibrate_pure_supported_subsystem_still_removes_stale_expose`, `::test_calibrate_unsupported_exposes_surfaced_as_needs_review`; `tests/validate/test_checks.py::test_drift_skips_declared_expose_for_unsupported_language_owner`, `::test_drift_supported_owner_not_in_unsupported_set_still_flags`; `tests/validate/test_engine.py::test_engine_no_structural_drift_for_unsupported_language_subsystem`.

### BOUNDS-023 — `calibrate --apply` re-enabled orphan-export floods by enriching bare consumes edges
- **Severity / Status:** medium / **Fixed**
- **Found:** 2026-06-05 via Supermemory dogfood: `bounds validate -H` had real cycle and coverage errors, then `bounds calibrate --apply` converted many draft/bare `consumes` edges into interface-level contracts and the next validation added a wall of `E_ORPHAN_EXPORT` warnings.
- **Affected:** `calibrate`/`validate`/`preflight` on repos whose manifests came from discovery or other subsystem-level mapping, especially large app repos with many exported components and utilities.
- **Symptom:** calibration looked like the recommended next action for drift, but after applying it, validation became noisier even though coverage gaps and cycles were unchanged. The extra warnings were not proof that the source got worse; they were a precision-mode side effect.
- **Root cause:** orphan detection is intentionally interface-level. It is meaningful only when a manifest says exactly which provider interfaces a consumer uses. Calibrate was auto-enriching bare edges generated by `discover`, turning a draft subsystem dependency into a curated interface contract without user intent.
- **Fix:** default calibration now preserves bare `consumes` edges as subsystem-level edges. It still removes stale provider edges and keeps already-curated interface edges current. Exact interface enrichment is explicit with `bounds calibrate --track-interfaces --apply`; recovery from accidental enrichment is explicit with `bounds calibrate --coarsen-interfaces --apply`, which drops interface lists but keeps provider edges. The human renderer shows interface additions/removals separately.
- **Test:** `tests/discover/test_calibrate.py::test_calibrate_preserves_existing_bare_edge_by_default`, `::test_calibrate_track_interfaces_enriches_existing_bare_edge`, `::test_calibrate_enriches_already_curated_interface_edge`, `::test_calibrate_coarsen_interfaces_removes_interface_lists_but_keeps_edges`.

### BOUNDS-024 — `discover --apply` could write invalid subsystem names from hidden/tooling dirs
- **Severity / Status:** medium / **Fixed**
- **Found:** 2026-06-05 via Firecrawl dogfood: a fresh `bounds discover --apply` wrote `.bounds/manifests/.github-scripts.yaml`, then `bounds validate` and `bounds calibrate` failed to load it with `E_USAGE invalid subsystem name '.github-scripts'`.
- **Affected:** `discover --apply` on repos where a hidden directory or punctuation-heavy path segment participates in a path-derived subsystem name, especially when basename collisions force path disambiguation.
- **Symptom:** discovery appeared to succeed, but the generated model was unreadable by Bounds itself. That breaks the one-command onboarding mental model: a write command must never create contracts the read path rejects.
- **Root cause:** the loader and `init` correctly restrict subsystem names to letters, digits, `-`, and `_`, but discovery used raw path segments when disambiguating names. Hidden segments like `.github` leaked dots into generated manifest names.
