...
git add .bounds && git commit -m "chore: adopt Bounds manifests"
...
3. **Commit `.bounds/`** so the contract is versioned with the code.
...
bounds describe billing         # what does billing rely on from auth? (verified, ~300 tokens)
