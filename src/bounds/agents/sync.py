...
def _sync(root: Path, selected: list[str]) -> dict:
    ...
    #     is told about Bounds from the file it ALWAYS reads — not only from an on-demand command/skill.
    ...
    for key in selected:
        ...
        outcome = _upsert_block(
            ...
            _MARKDOWN,
            _append_sdd_body(
                _append_invocation_directive(AGENT_POINTER_BODY.rstrip("\n"), invocation, _MARKDOWN),
                key, sdd_cfg,
            ),
        )
