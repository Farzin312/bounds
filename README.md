...
- **`bounds guide`** — a state-aware setup checklist (init → discover → coverage → wire agents → CI) for humans and agents, with SDD and invocation settings always visible; `bounds guide --sdd` previews the optional Spec-Driven Development track; `bounds --help` groups every command by purpose.
