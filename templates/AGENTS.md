<!--
Compact agent-instructions template — cross-tool standard (AGENTS.md).
...
the hidden `.compact/` directory (`.compact/manifests/<name>.yaml`); never read them
directly — use the CLI, which emits JSON by default.

Before making significant changes:

- `compact list` — discover the subsystems and how they connect.
- `compact describe <subsystem>` — the subsystem's public surface (`exposes`), its
  dependencies (`consumes`), and who depends on it (`consumed_by`). Read this instead of
  opening raw source to understand a boundary.
- `compact describe --namespace <ns>` — every subsystem in a domain at once.
- `compact overview` — project health: file counts, language breakdown, validation status.

After any change that touches exports or cross-subsystem imports:

- `compact validate --quick` — fast (sub-200ms) drift check. Treat a non-`fresh`
  `validation_status` as a signal to update the affected manifest, not to ignore.
- `compact preflight` — boundary, contract, cycle, and orphan checks before opening a PR.

Trust rule: when `validation_status` is `fresh`, rely on the manifest and skip the source
read. When it is `stale` or `unresolved`, run `compact validate` first, then trust.
